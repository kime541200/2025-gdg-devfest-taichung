from agents.mcp import MCPServer, MCPServerStdio, MCPServerSse, MCPServerStreamableHttp, ToolFilter, ToolFilterStatic
...
def _set_tool_filter(tool_filter_config: Optional[Union[ToolFilterStaticConfig, ToolFilterDynamicConfig]] = None) -> ToolFilter:
    ...
    if isinstance(tool_filter_config, ToolFilterStaticConfig):
        return ToolFilterStatic(allowed_tool_names=tool_filter_config.allowed_tool_names, blocked_tool_names=tool_filter_config.blocked_tool_names)
    elif isinstance(tool_filter_config, ToolFilterDynamicConfig):  
        import importlib          
